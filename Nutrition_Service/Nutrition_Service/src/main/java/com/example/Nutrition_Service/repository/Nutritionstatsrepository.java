...
public interface Nutritionstatsrepository extends JpaRepository<BesoinNutritionnel, Long> {
    ...
    @Query("SELECT a FROM Aliment a " +
            ...
            "AND (a.ageMinimumMois IS NULL OR :ageMois IS NULL OR a.ageMinimumMois <= :ageMois) " +
            "AND (:potassiumMax IS NULL OR a.potassiumMg IS NULL OR a.potassiumMg <= :potassiumMax) " +
            "AND (:sodiumMax IS NULL OR a.sodiumMg IS NULL OR a.sodiumMg <= :sodiumMax) " +
            "AND (:phosphoreMax IS NULL OR a.phosphoreMg IS NULL OR a.phosphoreMg <= :phosphoreMax) " +
            "AND (:sucreMax IS NULL OR a.sucreG IS NULL OR a.sucreG <= :sucreMax) " +
