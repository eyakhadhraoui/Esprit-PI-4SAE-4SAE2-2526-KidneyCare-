...
import com.example.nutrition_service.dto.NutritionStatsDTO.SuggestionAlimentsCriteria;
...
public interface NutritionStatsRepository extends JpaRepository<BesoinNutritionnel, Long> {
    ...
    @Query("SELECT a FROM Aliment a " +
            ...
            "  WHERE r.patientId = :#{#criteria.patientId()} AND r.dateFin IS NULL" +
            ") " +
            "AND (:#{#criteria.hasTacrolimus()} = false OR a.interactionTacrolimus = false) " +
            "AND (:#{#criteria.hasCyclosporine()} = false OR a.interactionCyclosporine = false) " +
            "AND (a.ageMinimumMois IS NULL OR a.ageMinimumMois <= :#{#criteria.ageMois()}) " +
            "AND (:#{#criteria.potassiumMax()} IS NULL OR a.potassiumMg <= :#{#criteria.potassiumMax()}) " +
            "AND (:#{#criteria.sodiumMax()} IS NULL OR a.sodiumMg <= :#{#criteria.sodiumMax()}) " +
            "AND (:#{#criteria.phosphoreMax()} IS NULL OR a.phosphoreMg <= :#{#criteria.phosphoreMax()}) " +
            "AND (:#{#criteria.sucreMax()} IS NULL OR a.sucreG <= :#{#criteria.sucreMax()}) " +
            "ORDER BY a.caloriesKcal ASC")
    List<Aliment> findAlimentsAutorisesForPatient(@Param("criteria") SuggestionAlimentsCriteria criteria);
