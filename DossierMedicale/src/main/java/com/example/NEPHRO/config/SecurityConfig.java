    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http, @Qualifier("corsConfigurationSource") CorsConfigurationSource corsConfigurationSource) throws Exception {
        http
            ...
            .authorizeHttpRequests(auth -> auth
                ...
                // Login/Register doivent rester accessibles même si la 1ère chaîne ne matche pas (proxy, path, etc.)
                .requestMatchers("/api/auth/**").permitAll()
                .requestMatchers("/uploads/**", "/error", "/actuator/health", "/actuator/info", "/actuator/prometheus", "/ws/**").permitAll()
