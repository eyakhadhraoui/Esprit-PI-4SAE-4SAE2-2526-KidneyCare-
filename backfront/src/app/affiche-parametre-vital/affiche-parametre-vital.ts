...
export class AfficheParametreVital implements OnInit {
  ...
  croissResult: Record<string, unknown> | null = null;
  ...
  immunoPhase = 'precoce';
  immunoResult: Record<string, unknown> | null = null;
  ...
  hydResult: Record<string, unknown> | null = null;
  ...
  fragiliteResult: Record<string, unknown> | null = null;
    ...
    this.http.get<Record<string, unknown>>(`${this.BASE_VITAL}/retrieveParametreVital/${id}`).subscribe({
      next: (data) => {
        this.editParametre = {
          ...
          valeurNormaleMin: (data['valeurNormaleMin'] as number | null) ?? (data['referenceMin'] as number | null) ?? null,
          valeurNormaleMax: (data['valeurNormaleMax'] as number | null) ?? (data['referenceMax'] as number | null) ?? null,
  ...
  saveEditParametre(): void {
    ...
    this.http.put(`${this.BASE_VITAL}/updateParametreVital/${this.editParametreId}`, this.editParametre).subscribe({
      next: () => {
  ...
  calcFragilite(): void {
    ...
    let score = 0;
    const criteres: Array<Record<string, unknown>> = [];
  ...
  saveEditConstante(): void {
    ...
    const payload = {
      nomParametre: this.editConstante.nomParametre,
      unite: this.editConstante.unite,
      valeurMinNormale: this.editConstante.valeurMinNormale,
      valeurMaxNormale: this.editConstante.valeurMaxNormale,
      poidsMin: this.editConstante.poidsMin,
      poidsMax: this.editConstante.poidsMax,
      tailleMin: this.editConstante.tailleMin,
      tailleMax: this.editConstante.tailleMax,
      indicateurVital: this.editConstante.indicateurVital,
    };
  ...
  saveEditIndicateur(): void {
    ...
    this.http.put(`${this.BASE_INDICATEUR}/updateIndicateurVital/${this.editIndicateurId}`, this.editIndicateur).subscribe({
      next: () => {
