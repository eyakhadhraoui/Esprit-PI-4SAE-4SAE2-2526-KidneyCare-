...
export class ConsultationComponent implements OnInit {
  ...
  loadPatients(): void {
    this.http.get<PatientDTO[]>(`${BASE}/patient/retrievePatients`).subscribe({
      ...
      error: () => {
        // Fallback si projetconsultation (8081) est arrêté : prescription-Service (8086)
        this.http.get<PatientDTO[]>(`/prescription/patients`).subscribe({
          next: (data2) => {
            this.patientsList = data2 || [];
          },
          error: (err2) => {
            console.error('Erreur patients:', err2);
            this.patientsList = [];
          },
        });
      },
