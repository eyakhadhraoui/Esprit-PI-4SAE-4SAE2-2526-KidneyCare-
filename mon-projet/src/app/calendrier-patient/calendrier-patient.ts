...
export class CalendrierPatientComponent implements OnInit {
  ...
  private typeOrder(t: string): number {
  ...
  getEventsForDate(dateKey: string): CalendrierEvent[] {
    ...
    return [...list].sort((a, b) => {
      const oa = this.typeOrder(a.type);
      const ob = this.typeOrder(b.type);
  ...
  get countTestsDemandes(): number { return this.events.filter(e => e.type === 'TEST_DEMANDE').length; }
  get countTestsRealises(): number { return this.events.filter(e => e.type === 'TEST_REALISE').length; }
  get countRapports(): number { return this.events.filter(e => e.type === 'RAPPORT_BILAN').length; }

  filterTypeLabel(): string {
    switch (this.filterType) {
      case 'SUIVI': return 'Follow-ups';
      case 'IMAGE_MEDICALE': return 'Images';
      case 'TEST_DEMANDE': return 'Test requests';
      case 'TEST_REALISE': return 'Tests completed';
      case 'RAPPORT_BILAN': return 'Medical reports';
      default: return '';
    }
  }
  ...
  /** Indique si une date (jour du mois) a des événements visibles avec le filtre courant. */
  hasEventsOnDay(day: number): boolean {
    ...
    return list.some(e => e.type === this.filterType);
  }
  ...
  getTypeLabel(type: string): string {
    if (type === 'IMAGE_MEDICALE') return 'Medical image';
    if (type === 'TEST_DEMANDE') return 'Test requested';
    if (type === 'TEST_REALISE') return 'Test completed';
    if (type === 'RAPPORT_BILAN') return 'Medical report';
    return 'Follow-up';
  }

  getTypeIcon(type: string): string {
    if (type === 'IMAGE_MEDICALE') return '🖼️';
    if (type === 'TEST_DEMANDE') return '📝';
    if (type === 'TEST_REALISE') return '🧪';
    if (type === 'RAPPORT_BILAN') return '📄';
    return '📋';
  }
