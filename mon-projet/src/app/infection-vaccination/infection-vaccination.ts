...
import {
  ...
  Infection,
  Vaccination
} from '../services/infection-vaccination.service';
  ...
  loadAll() {
    ...
    this.svc.getAllVaccinations().subscribe({
      next: (data: Vaccination[]) => {
        this.vaccinations = data.map((v: Vaccination) => ({ ...v, booster_taken: v.booster_taken ?? false }));
        this.cdr.detectChanges();
      },
      error: (err: unknown) => console.error('Vaccinations load error:', err)
    });
  }
  ...
  saveInfection() {
    ...
    if (this.editMode && this.editingInfId !== null) {
      this.svc.updateInfection(this.editingInfId, this.newInfection).subscribe({
        next: () => { this.loadAll(); this.closeForm(); }, error: (err: unknown) => console.error(err)
      });
    } else {
    ...
    if (this.editMode && this.editingVacId !== null) {
      ...
      this.svc.deleteVaccination(this.editingVacId).subscribe({
        next: () => this.svc.createVaccination({ ...payload, booster_taken }).subscribe({
          next: () => { this.loadAll(); this.closeVaccinationForm(); }, error: (err: unknown) => console.error(err)
        }), error: (err: unknown) => console.error(err)
      });
    } else {
