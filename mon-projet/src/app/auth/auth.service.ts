...
export class AuthService {
  ...
  loginWithCredentials(username: string, password: string): Promise<void> {
    return firstValueFrom(
      ...
      // status 0 : pas de réponse HTTP (proxy injoignable, backend arrêté, CORS, ou ng serve sans proxy)
      let msg = err?.error?.message ?? err?.message ?? 'Erreur de connexion';
      if (err?.status === 0) {
        msg =
          'Serveur injoignable. Démarrez le backend NEPHRO (port 8089), puis lancez `ng serve` depuis le dossier mon-projet ' +
          '(proxy.conf.js actif). Vérifiez aussi que Keycloak tourne si le login l’utilise.';
      }
