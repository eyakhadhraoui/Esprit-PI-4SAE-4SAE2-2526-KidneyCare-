...
export class ConsultationService {
  ...
  getPatients(): Observable<PatientDTO[]> {
    // 1) Source principale : projetconsultation (8081)
    // 2) Fallback : prescription-Service (8086) via proxy /prescription (utile si 8081 est arrêté)
    return this.http.get<PatientDTO[]>(`${BASE}/patient/retrievePatients`).pipe(
      catchError(() => this.http.get<PatientDTO[]>(`/prescription/patients`).pipe(
        catchError(() => of([]))
      ))
    );
  }
