...
export interface RapportBilanDTO {
  ...
  /** Si true, le backend envoie un email au patient après création. */
...
export interface LabAnalysisResponse {
  ...
  /** Conclusion texte générée par Gemini / Anthropic si ?aiConclusion=true et clé API configurée côté backend. */
  ai_conclusion?: string;
  ai_provider?: string;
  ai_conclusion_error?: string;
  stats?: {
...
export class ModuleLaboService {
  ...
  /** PDF complet (médecin) : traitement + résultats pour une demande. */
  getPdfCompletPrescription(prescriptionId: number): Observable<Blob> {
    return this.http.get(`${this.base}/prescriptions-bilan/${prescriptionId}/pdf-complet`, { responseType: 'blob' }).pipe(
      catchError(e => throwError(() => ({ message: e?.error?.message ?? e?.message ?? 'Erreur PDF' })))
    );
  }
  ...
  /**
   * @param aiConclusion si true : POST /api/lab-analysis?aiConclusion=true (Gemini prioritaire, sinon Anthropic si configuré).
   */
  analyzeLab(payload: LabAnalysisPayload, aiConclusion = true): Observable<LabAnalysisResponse> {
    let params = new HttpParams();
    if (aiConclusion) params = params.set('aiConclusion', 'true');
    const opts = params.keys().length ? { params } : {};
    return this.http.post<LabAnalysisResponse>(`${this.base}/lab-analysis`, payload, opts).pipe(
